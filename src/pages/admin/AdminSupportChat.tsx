...
const AdminSupportChat = () => {
  ...
  const sendMessage = useMutation({
    ...
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["admin-support-chat", selectedProfId] });
      setMessage("");
    },
  ...
  // Sort professionals: those with conversations first
  ...
  return (
    <AdminLayout title="Chat de Suporte" subtitle="Responder mensagens de suporte dos usuários">
        ...
        <div className="w-80 glass-card rounded-2xl flex flex-col overflow-hidden shrink-0 hidden md:flex">
          <div className="p-3 border-b border-border">
            <input
              ...
              className="w-full px-3 py-2 rounded-xl bg-muted border border-border text-sm text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-accent/30"
            />
          </div>
                ...
                return (
                  <button
                    ...
                    className={cn(
                      ...
                      selectedProfId === p.id && "bg-accent/10"
                    )}
                  >
                    <div className="flex items-center gap-3">
                      <div className={cn(
                        "w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold shrink-0",
                        hasConvo ? "bg-accent/20 text-accent" : "bg-muted text-muted-foreground"
                      )}>
                        <User size={14} />
                      </div>
        ...
        <div className="flex-1 glass-card rounded-2xl flex flex-col overflow-hidden">
          {!selectedProfId ? (
            <div className="flex-1 flex flex-col items-center justify-center text-center p-4">
              ...
              <p className="text-sm text-muted-foreground">Selecione um usuário para ver a conversa</p>
            </div>
          ) : (
            <>
                          ...
                          {msg.attachment_url && (
                            <a href={msg.attachment_url} target="_blank" rel="noopener noreferrer" className="mt-2 block">
                              <div className="flex items-center gap-2 bg-background/20 rounded-lg p-2 hover:bg-background/30 transition-colors">
                                <ImageIcon size={14} /><span className="text-xs underline">Ver arquivo</span>
                              </div>
                            </a>
                          )}
